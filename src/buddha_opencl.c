...
void mandelIterOpenCL(params_t* p_params){
	// Allocate memories
	...
	kernelFile = fopen(p_params->kernel_filename, "r");

	if (!kernelFile) {
		fprintf(stderr, "No file named %s was found !\n", p_params->kernel_filename);
		exit(-1);
	}
	...
	while(1){
		...
		int x, y;
		complex trajs[p_params->n_kernels][p_params->maxiter];
		for(int i = 0; i < p_params->n_kernels; i++){
			for(int j = 0; j < p_params->maxiter; j++){
				...
				y = (int)map(trajectoriesA[j + p_params->maxiter * i], -0.35 * sqrt(2), 0.65 * sqrt(2), 0, p_params->resy);
				trajs[i][j] = (complex) {trajectoriesA[j + p_params->maxiter * i],
					       		 trajectoriesB[j + p_params->maxiter * i]};
